...
function buildDeployChain(options) {
  ...
  return Client.builder().use((context) => {
    context.transaction_attributes = context.source_code;
    return context;
  }).use(new TransactionCreator(apiConfig)).use(
    (context) =>
      tagger.handle(__spreadProps(__spreadValues({}, context), {
        tags: createSourceCodeTransactionTags(),
      })),
  ).use(new TransactionSigner(apiConfig)).use(new TransactionPoster(apiConfig))
    .use((context) => {
    ...
    ).use(new TransactionSigner(apiConfig)).use(
      new TransactionPoster(apiConfig),
    ).use((context) => {
...
var SmartWeaveContractInteractReader = class extends TransactionHandler {
  handle(context) {
    return Promise.resolve().then(() => {
      var _a, _b, _c, _d;
      return interactRead(
...
function buildReaderChain(options) {
  const apiConfig = options.api_config || {};
  return Client.builder().use(new SmartWeaveContractInteractReader(apiConfig))
    .use((context) => {
      return {
...
function buildViewerChain(options) {
  const apiConfig = options.api_config || {};
  return Client.builder().use(new SmartWeaveContractViewer(apiConfig)).use(
    (context) => {
      return {
        read_result: context.read_result,
      };
...
function buildWriterChain(options) {
  const apiConfig = options.api_config || {};
  return Client.builder().use((context) => {
    context.transaction_attributes = __spreadProps(
      __spreadValues({}, context.transaction_attributes),
      {
